...
secondfile = masterfile.split('\n') #splits string into list of smaller strings
first_wire = secondfile[0].split(",")
second_wire = secondfile[1].split(",")
#print(first_wire)
#print(second_wire)

#---MAKING MATRIX OF 0's assuming size
n = 100000
m = 100000
matrix = [0] * n
for i in range(n):
...
def follow_wire(str):
    ...
    if direction == "U":
        for i in range(0,steps):
            matrix[ncurrent][mcurrent + i] = matrix[ncurrent][mcurrent + i] + 1
    if direction == "D":
        for i in range(0,steps):
            matrix[ncurrent][mcurrent - i] = matrix[ncurrent][mcurrent - i] + 1
    if direction == "R":
        for i in range(0,steps):
            matrix[ncurrent + i][mcurrent] = matrix[ncurrent + i][mcurrent] + 1
    if direction == "L":
        for i in range(0,steps):
            matrix[ncurrent - i][mcurrent] = matrix[ncurrent - i][mcurrent] + 1

follow_wire(first_wire)
follow_wire(second_wire)

#---ANSWER (find closest 2)
distances=[]
for i in n:
    for j in m:
        if matrix[n][m]==2:
            distances.append(abs(n/2-n) + abs(m/2-m))
print(min(distances)) # Manhattan distance from port to closest intersection